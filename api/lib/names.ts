/**
 * api/lib/names.ts — ported from backend/src/utils/names.ts
 */

import type { Machine, Recipe, StockMap } from './types.js';
...
export function findNameConflict(
  ...
  const recipe = recipes.find(
    (r) => r.id !== excludeRecipeId && normalizeName(r.name) === normalized
  );
  if (recipe) return { type: 'สินค้า', name: recipe.name };

  const machine = machines.find(
    (m) =>
      m.machine_id !== excludeMachineId &&
      normalizeName(m.machine_name) === normalized
  );
  if (machine) return { type: 'เครื่องจักร', name: machine.machine_name };

  const recipeIds = new Set(recipes.map((r) => r.id));
  const stockName = Object.keys(stocks).find(
    (key) => !recipeIds.has(key) && normalizeName(key) === normalized
  );
